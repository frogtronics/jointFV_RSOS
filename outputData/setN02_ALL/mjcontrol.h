...
struct _mjControl
{
    ...
    double error_tol = 0.001;//default 0.001
    ...
    double reach_time_factor = 1; // value to be overwritten in main code
    ...
    //double K[6] = { 4342.35, 1.0816 * sqrt(4342.35), 5154.67, 0.0051 * sqrt(5154.67), 1622.29, 0.0090 * sqrt(1622.29) };
    ...
    //double K[6] = { 853.76, 1.3384 * sqrt(853.76), 743.07, 0.4673 * sqrt(743.07), 270.68, 0.105 * sqrt(270.68) };//CHRIS USE THIS
    ...
    //double K[6] = { 76312.34, 6.4059 * sqrt(76312.34),  79309.97, 7.4841 * sqrt(79309.97),   73724.74, 6.4667 * sqrt(73724.74) };//default
    ...
    //------------------SLOW REACH 3RD ORDER ACT DYNAMICS + FVL + computed torque------------------------
    //parameters re-optimised by Tiina, 18.12.23
    //optimised for reach factor = 1.712 (control)
    // int act_mode = 0;//0 for 3rd order, 1 for 1st order
    // double delay = 28;
    // double K[6] = { 1634.37, 1.6868 * sqrt(1634.37), 1598.33, 1.684 * sqrt(1598.33), 1146.1, 2.0282 * sqrt(1146.1) };

    //------------------FAST REACH 3RD ORDER ACT DYNAMICS + FVL + computed torque------------------------
    //parameters re-optimised by Tiina, 18.12.23
    //optimised for reach factor = 0.43 (4x speed)
    int act_mode = 0;//0 for 3rd order, 1 for 1st order
    double delay = 25;
    double K[6] = { 684.45, 1.6064 * sqrt(684.45), 3685.82, 1.5886 * sqrt(3685.82), 6423.55, 1.1522 * sqrt(6423.55) }; 

    //------------------SLOW REACH 1ST ORDER ACT DYNAMICS + FVL + computed torque------------------------
    //parameters re-optimised by Tiina, 18.12.23
    //optimised for reach factor = 1.712 (control)
    // int act_mode = 1;//0 for 3rd order, 1 for 1st order
    // double delay = 6;
    // double K[6] = { 95236.09, 2.0853 * sqrt(95236.09), 64175.31, 2.3015 * sqrt(64175.31), 19537.03, 5.19 * sqrt(19537.03) };                    

    //------------------FAST REACH 1ST ORDER ACT DYNAMICS + FVL + computed torque------------------------ 
    //parameters re-optimised by Tiina, 18.12.23
    //optimised for reach factor = 0.43 (4x speed)
    // int act_mode = 1;//0 for 3rd order, 1 for 1st order
    // double delay = 6;
    // double K[6] = { 34955.02, 4.5685 * sqrt(34955.02), 47609.59, 5.0843 * sqrt(47609.59), 85439.72, 3.9995 * sqrt(85439.72) };    

    /* ADJUSTIBLE WITHIN XML GEOMTRY LIMITS */
    ...
    mjtNum* PDpert;//addedf by CTR for injecting a perturbation to the PD control vector
    mjtNum* PDprev;//PDmagn at previous time step
    mjtNum* computed_torqueFF;//computed torque opened from saved file from a Feedback simulation
    ...
    void init(const int nv)
    {
        ...
        PDpert = new mjtNum[nv];
        PDprev = new mjtNum[nv];
        computed_torqueFF = new mjtNum[nv];
    }
